...
void setup() {
  ...
  // Read the data file...
  Table myTable = loadTable("AAPLStock.csv", "header");

  // How long is the dataset
  price = new float[myTable.getRowCount()];
  ...
  for (TableRow row : myTable.rows()) {
    ...
    price[i] = row.getFloat("Close");
    mm[i] = row.getInt("Month");
    i++;
  }
  println("Data Loaded: " + myTable.getRowCount() + " entries.");
