...
void setup() {
   ...
   * at an audio rate.
   */
  Glide freqEnv = new Glide(ac, 500);
  freqEnv.setGlideTime(200);
  /*
   ...
   */
  WavePlayer wp = new WavePlayer(ac, freqEnv, Buffer.TRIANGLE);
  /*
   * So now that the WavePlayer is set up with the 
   * frequency Glide. Note that when we made the Envelope
   * it was set to 500, so the transition goes from 500 to
   * 1000. 
   */
   
  
  freqEnv.setValue(1000);
  /*
