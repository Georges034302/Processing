...
void setup() {
  ...
  // A Low Frequency Oscillator controlling a gain to add tremelo
  WavePlayer lfo = new WavePlayer(ac, 1, Buffer.SINE);
  Gain lfoGain = new Gain(ac, 1, lfo);
  
  // A Gain just for volume control;
  Gain g = new Gain(ac, 1, 0.1);
    
  /*
   * Connect it all together but with the lfo first in the chain.
   */
  lfoGain.addInput(wp);
  g.addInput(lfoGain);
  ac.out.addInput(g);
